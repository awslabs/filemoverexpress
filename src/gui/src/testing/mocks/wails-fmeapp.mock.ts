// Mock for generated Wails bindings (@gen/wails/FileMoverExpressUI/fmeapp)

export const AppVersion = () => Promise.resolve('');
export const ExternalLink = (__url: string) => Promise.resolve();
export const FatalShutdown = () => Promise.resolve();
export const FirstLaunchComplete = () => Promise.resolve(true);
export const HandleBeforeClose = (__e: unknown) => Promise.resolve();
export const StartDaemon = () => Promise.resolve();
export const SaveFile = (__defaultFilename: string, __base64Data: string) => Promise.resolve('');
export const SystemOpen = (__path: string) => Promise.resolve();
export const SystemShowItemInFolder = (__path: string) => Promise.resolve();
export const GenerateExcelReport = (__data: unknown) => Promise.resolve('');
export const GenerateCsvReport = (__data: unknown) => Promise.resolve('');
export const GenerateJsonReport = (__data: unknown) => Promise.resolve('');
